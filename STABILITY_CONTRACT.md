...
| `type` | Single byte | One of: `MESSAGE_TYPE_DATA` (0x00), `MESSAGE_TYPE_CONFIG` (0x01), `MESSAGE_TYPE_APP_PING` (0x02), `MESSAGE_TYPE_APP_PONG` (0x03), `MESSAGE_TYPE_CONFIG_RESPONSE` (0x05), `MESSAGE_TYPE_COMMAND` (0x06) |
...
- **2026-09-07** — RWT-PROTO-001 amended. Added `MESSAGE_TYPE_COMMAND` (0x06) to protocol type table. Server can now send commands to clients via `sendCommand()` function or HTTP API. Client emits `command` event. Regression tests: `messageHandler.test.js` (3 tests).
- **2026-09-07** — RWT-PROTO-001 amended. Added `MESSAGE_TYPE_CONFIG_RESPONSE` (0x05) to protocol type table. Server now sends CONFIG_RESPONSE with `serverVersion` after processing CONFIG. Client emits `serverVersion` event. Regression tests: `messageHandler.test.js` (2 tests).
